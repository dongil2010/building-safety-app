...
요청 시 **항상** 아래 순서를 자동으로 진행합니다 (커밋 확인 → 풀 → 커밋 → 풀 → 푸시 상시):
...
9. **배포는 GitHub Pages** — 현장 Android 앱은 Capacitor WebView로
...
- **상시 동기화**: 의미 있는 코드 변경을 마치면 별도 지시 없이도 표준 루틴(커밋 확인 → pull → commit → pull → push)을 수행
