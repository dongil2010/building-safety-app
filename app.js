...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function calcConcreteStrength(readings, angleDeg, ageDays, enabledFormulaNames) {
        ...
        // 전부 해제) 평균이 안 나오니 안전하게 3개 전부로 되돌려 계산한다. 준공일이 없어 재령을
    ...
    // 위치별로 R값 목록을 따로 관리할 수 있게 슬롯을 둔다. 슬롯 1개(빈 위치)가 기본값.
    ...
    function renderNdtStrengthSlots() {
        ...
        container.innerHTML = ndtStrengthSlots.map((slot, idx) => `
            <div style="border:1px solid var(--border-color); border-radius:10px; padding:0.7rem; display:flex; flex-direction:column; gap:0.6rem;">
                <div style="display:flex; gap:0.5rem; align-items:flex-end;">
                    <div class="form-group" style="flex:1; margin:0;">
                        <label class="form-label">📍 측정 위치 ${ndtStrengthSlots.length > 1 ? `(${idx + 1})` : ''}${idx === 0 ? ' *' : ''}</label>
                        <input type="text" class="form-control ndt-strength-slot-location" data-slot="${idx}" value="${slot.location || ''}" placeholder="예: 101동 1F 기둥 C1 하부(플렌지)">
                    </div>
                    ${ndtStrengthSlots.length > 1 ? `<button type="button" class="btn btn-sm btn-outline ndt-strength-slot-remove" data-slot="${idx}" style="border-color:#ef4444; color:#ef4444;" title="이 위치 삭제"><i class="fa-solid fa-trash"></i></button>` : ''}
                </div>
                    ...
                    <table style="width:100%; border-collapse:collapse; font-size:0.8rem;">
                        <thead>
                            <tr style="text-align:left; color:var(--text-muted); border-bottom:1px solid rgba(2,132,199,0.3);">
                                <th style="padding:0.25rem 0.4rem; width:1.6rem;">포함</th>
    ...
    function getCurrentFloorFilteredDefects() {
        ...
        if (catFilter) {
            list = list.filter(d => {
                ...
                const group = getStructuralComponentGroup(d.component);
                if (group === 'both') return catFilter.columnWall !== false || catFilter.beamSlab !== false;
                if (group === 'columnWall') return catFilter.columnWall !== false;
                if (group === 'beamSlab') return catFilter.beamSlab !== false;
    ...
    function isCategoryFilterOn(catKey) {
        const f = window.state.categoryFilter || {};
        if (catKey === 'columnWall') return f.columnWall !== false;
        if (catKey === 'beamSlab') return f.beamSlab !== false;
        if (catKey === 'nonStructural') return f.nonStructural !== false;
        if (catKey === 'finishing') return f.finishing !== false;
        return true;
    }

    function setCategoryFilterOn(catKey, on) {
        if (!window.state.categoryFilter) {
            window.state.categoryFilter = { columnWall: true, beamSlab: true, nonStructural: true, finishing: true };
        }
        window.state.categoryFilter[catKey] = !!on;
        const checkboxIdMap = {
            columnWall: 'filterCatColumnWall',
            beamSlab: 'filterCatBeamSlab',
            nonStructural: 'filterCatNonStructural',
            finishing: 'filterCatFinishing'
        };
        const el = document.getElementById(checkboxIdMap[catKey]);
        if (el) el.checked = !!on;
    }
    ...
    function renderDefectListPanel(options = {}) {
        ...
        if (summaryEl) {
            ...
            allFloorDefects.forEach(d => {
                const cat = d.category || '구조체';
                if (cat === '비구조체') {
                    counts.nonStructural++;
                    return;
                }
                if (cat === '마감재') {
                    counts.finishing++;
                ...
                if (group === 'both') {
                    counts.columnWall++;
                    counts.beamSlab++;
                } else if (group === 'columnWall') {
                    counts.columnWall++;
                } else if (group === 'beamSlab') {
                    counts.beamSlab++;
                }
    ...
    function ensureAreaPinPlacement(defect) {
        ...
        const atLegacyCorner = Math.abs((defect.x ?? x1) - x1) < 2 && Math.abs((defect.y ?? y1) - y1) < 2;
        const atLegacyCenter = Math.abs((defect.targetX ?? 0) - cx) < 2
            && Math.abs((defect.targetY ?? 0) - cy) < 2;
        const needsTip = defect.targetX === undefined || defect.targetY === undefined
            || Number.isNaN(Number(defect.targetX)) || Number.isNaN(Number(defect.targetY));
        const needsBox = defect.x === undefined || defect.y === undefined
    ...
    function isDefectComboCustomToken(val) {
        return !val || String(val).startsWith('__ADD_CUSTOM');
    }

    function ensureDefectComboOption(select, value) {
        if (!select || !value || isDefectComboCustomToken(value)) return;
        const exists = Array.from(select.options).some(o => o.value === value);
        if (!exists) {
            const customOpt = document.createElement('option');
            customOpt.value = value;
            customOpt.textContent = value;
            const addOpt = select.querySelector('option[value^="__ADD_CUSTOM"]');
            if (addOpt) select.insertBefore(customOpt, addOpt);
            else select.appendChild(customOpt);
        }
        select.value = value;
    }

    function syncDefectComboFields(select, input, value) {
    ...
    function setDefectArrowOctant(octant) {
        ...
        document.querySelectorAll('#defectArrowOctantGrid .arrow-octant-btn').forEach(btn => {
    ...
    function syncDefectArrowDirUi() {
        const forceArrowDirEl = document.getElementById('defectForceArrowDir');
        const grid = document.getElementById('defectArrowOctantGrid');
        const enabled = !!(forceArrowDirEl && forceArrowDirEl.checked);
        if (grid) {
    ...
    function bindDefectArrowOctantGrid() {
        ...
        grid.addEventListener('click', (e) => {
            const btn = e.target.closest('.arrow-octant-btn');
            if (!btn || btn.disabled) return;
            setDefectArrowOctant(btn.dataset.octant);
        });
        if (forceArrowDirEl && !forceArrowDirEl.dataset.boundArrowDirToggle) {
            forceArrowDirEl.addEventListener('change', syncDefectArrowDirUi);
            forceArrowDirEl.dataset.boundArrowDirToggle = '1';
        }
        setDefectArrowOctant(document.getElementById('defectArrowOctant')?.value || 0);
    ...
    function isDefectModalOpen() {
        const modal = elements.defectModal || document.getElementById('defectModal');
        if (!modal) return false;
        return modal.classList.contains('open') || modal.style.display === 'flex';
    }
    ...
    function openAddDefectModal(boxX, boxY, targetX, targetY, existingPin = null, areaRect = null) {
        ...
        if (existingPin) {
            ...
            if (areaRect) {
                ...
                const cx = (ax1 + ax2) / 2;
                const cy = (ay1 + ay2) / 2;
                const areaCat = (tmpl && tmpl.category) || document.getElementById('defectCategory')?.value || '구조체';
                const areaNo = noEl?.value || 'NO.01';
                const areaStyleKey = getDefectStyleKey(areaCat, tmpl ? (tmpl.defectType || '') : '');
    ...
    function scheduleDefectAutoApply() {
        ...
        if (!isDefectModalOpen()) return;
        window.clearTimeout(window._defectAutoApplyTimer);
        window._defectAutoApplyTimer = window.setTimeout(async () => {
            if (window._defectFormHydrating || !isDefectModalOpen()) return;
            flashDefectAutosaveBadge('saving');
    ...
    function bindDefectFormAutoApply() {
        ...
        if (!form || form.dataset.autoApplyBound) return;
    ...
    function parseSurveyDefectNo(d) {
        const raw = String((d && d.no) || '').replace(/^NO\.?\s*/i, '').trim();
        const m = raw.match(/(\d+)(?:-(\d+))?/);
        const main = m ? parseInt(m[1], 10) : 0;
        const suffix = m && m[2] ? parseInt(m[2], 10) : 1;
        return { main, suffix, extra: suffix >= 2 };
    }
    ...
    function paginateSurveyDefects(list) {
        const BASE = SURVEY_REPORT_ROWS_BASE;
        const MAX = SURVEY_REPORT_ROWS_MAX;
        if (!list || !list.length) return [[]];
        const pages = [];
        let i = 0;
        let closedMultiple = 0;
        while (i < list.length) {
            const target = closedMultiple + BASE;
            let count = 0;
            for (let k = i; k < list.length; k++) {
                if (parseSurveyDefectNo(list[k]).main > target) break;
                count++;
            }
            if (count === 0) {
    ...
    function getSurveyStructMarks(d, isGrade3) {
        const on = isGrade3 ? 'O' : '○';
        const isStruct = d.category === '구조체';
        return { struct: isStruct ? on : '-', nonstruct: isStruct ? '-' : on };
    }
        ...
        try {
            ...
            targets.forEach(t => {
                ...
                if (!isAreaDefectSafe) {
                    ...
                    if (useCircleTipSafe) {
                        ctx.beginPath();
                        ctx.arc(tipX, tipY, 4.5 * safeArrowScale, 0, Math.PI * 2);
                        ctx.fill();
                    } else {
                        const angle = Math.atan2(leader.uy, leader.ux);
        ...
        try {
            ...
            for (let slotIdx = 0; slotIdx < floorsData.length; slotIdx++) {
                ...
                if (dataRows.length < 1) {
                    console.error(`${floorCode}: 템플릿 표의 데이터 행 스타일을 판별할 수 없어 이 층은 건너뜁니다.`);
                    continue;
                }
                ...
                // 표 하나에 행을 전부 몰아넣으면 한글에서 다음 페이지로 안 넘어가거나 열람 중
                // 멈추는 문제가 있었다. 15행(최대 17, n-n 추가분)마다 표를 나누고, 가능하면
                // 본번호가 15·30·45에서 끝나게 한다.
                const surveyPages = paginateSurveyDefects(pageDefects);
                const neededPages = Math.max(1, surveyPages.length);
