...
window.BSA.tabs['tab-map'] = {
    ...
    features: [
        ...
        '화살표 추가: 같은 번호칸에 화살표·영역만 추가 (조사표 행은 만들지 않음)',
        '마킹 X-1/X-2 탭: PC는 수정창 왼쪽, 모바일은 수정창 위에 붙임',
        '영역 내부 그리기: 선·박스·다각형·타원·자유선 (선택 영역 안)',
        '영역 각도 회전 핸들·모달 각도 저장',
        '다각형 다시그리기: 저장된 영역 외곽을 기존 다각형 점찍기 UX로 교체 (번호·사진·내용 유지)',
        'Ctrl+드래그·클릭 추가/해제 다중 선택 · 그룹 이동 · 선택 삭제',
        'Delete/Backspace: 선택 마킹 삭제 · 수정창이 열린 핀도 삭제',
        '모바일·태블릿: 마킹 0.5초 홀딩 후 드래그 (살짝 밀면 도면 팬)',
        '벡터 PDF 내보내기 (현재 층 / 전층 한 PDF)',
        '결함 상세 모달 (사진·칩 입력, 하단은 위치·체크·화살표만)',
        '개구부 균열 체크 · 결함목록에서 켜고 끄기 · 조사표·한글 조사내용에 개구부 주위 ○○균열 표기',
        '결함표 추가: 미번호 화살표에서 「번호 부여」 → 표·한글 N-1·N-2 (표당 15·최대 17)',
        '결함 종류 칩: 부재별 프리셋(비구조·마감 포함, 벽체 접합부) · 복수 선택(선택 순) · 원인 연동',
        '발생 원인 복수 체크(복합 원인) · 벽체 접합부: 이질재료 거동차이·시공미흡 등',
        '균열 폭·길이·개수를 한 행에서 입력(행 추가/삭제) · /·x 조인 토글',
        '결함 창: 전차/현차 사진 가로 배치, 추가 사진은 아래 적재',
        '결함 사진 저장 장축 2000px (4:3 · 2000x1500)',
        '전차 사진 IndexedDB/클라우드 저장',
        '태블릿: 상단 탭·우측 레일·하단 독을 모바일 툴바 디자인으로 통일',
        '모바일 세로: 결함 수정 창 하단 1/2 · 태블릿/PC는 도면 오른쪽 슬라이드',
        '결함목록: 그룹을 1 · 1-2 · 1-3 원 번호로 하나씩 표시 · 현차·전차 구역 독립 스크롤',
        ...
        '조사항목(좌측 목록) 창 ON/OFF · OFF 시 선택 결함을 도면 좌상단 팝업(내용·폭)으로 표시',
        ...
        'CAD 핀: 전차 미표기(미등록) 마킹이 있으면 조사내용을 캐드 위치에 배치',
